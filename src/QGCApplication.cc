      ...
      _systemId(cli.systemId.value_or(0))
{
    ...
    if (_runningUnitTests || _simpleBootTest) {
        ...
        // Display name doubles as the QSettings space. Unlike upstream there is no
        // daily suffix: this fork ships a single stream, so there is no stable
        // install of the same application to keep settings apart from.
